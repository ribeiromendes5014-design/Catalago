...
def cadastrar_cliente_cashback(df_clientes, nome, contato_limpo, nivel='Prata'):
    novo_cliente = {
        'NOME': nome, 'APELIDO/DESCRIÇÃO': '', 'CONTATO': contato_limpo,
        'CASHBACK_DISPONIVEL': 0.00, 'GASTO_ACUMULADO': 0.00, 'NIVEL_ATUAL': nivel,
        'INDICADO_POR': '', 'PRIMEIRA_COMPRA_FEITA': 'FALSE', 'CONTATO_LIMPO': contato_limpo
    }
...
def calcular_cashback_a_creditar(pedido_json, df_catalogo):
    ...
    if not pedido_str or pedido_str.lower() in ('nan', '{}', ''): return 0.0
    try:
        try: detalhes_pedido = json.loads(pedido_str)
        except (json.JSONDecodeError, TypeError): detalhes_pedido = ast.literal_eval(pedido_str)
        itens = detalhes_pedido.get('itens', [])
        for item in itens:
            try: item_id = int(item.get('id', -1))
            except (TypeError, ValueError): continue
            cashback_percent_str = str(item.get('cashbackpercent', 0)).replace(',', '.')
            try: cashback_percent = float(cashback_percent_str)
            except ValueError: cashback_percent = 0.0
            if cashback_percent == 0.0 and not df_catalogo.empty:
                ...
                if not produto_catalogo.empty:
                    ...
                    try: cashback_percent = float(catalogo_cashback_str)
...
def atualizar_status_pedido(id_pedido, novo_status, df_catalogo):
    ...
    if not index_to_update.empty:
        ...
        if novo_status == 'Finalizado' and df.loc[idx, 'STATUS'] != 'Finalizado':
            pedido = df.loc[idx]
            valor_cashback_credito = calcular_cashback_a_creditar(pedido.get('ITENS_JSON'), df_catalogo)
            if pedido.get('ITENS_JSON') and pedido.get('CONTATO_CLIENTE') and valor_cashback_credito > 0:
                if not lancar_venda_cashback(pedido.get('NOME_CLIENTE'), pedido.get('CONTATO_CLIENTE'), valor_cashback_credito):
...
def exibir_itens_pedido(id_pedido, pedido_json, df_catalogo):
    try:
        ...
        if not pedido_str or pedido_str.lower() in ('nan', '{}', ''):
            ...
            return 0
        try: detalhes_pedido = json.loads(pedido_str)
        except json.JSONDecodeError: detalhes_pedido = ast.literal_eval(pedido_str)
        itens = detalhes_pedido.get('itens', [])
        ...
        for i, item in enumerate(itens):
            ...
            if not df_catalogo.empty and not pd.isna(item_id) and not df_catalogo[df_catalogo['ID'] == int(item_id)].empty: 
                ...
                if link_na_tabela.lower() != 'nan' and link_na_tabela: link_imagem = link_na_tabela
            col_check, col_img, col_detalhes = st.columns([0.5, 1, 3.5])
            checked = col_check.checkbox(label=" ", value=st.session_state[key_progress][i], key=f"check_{id_pedido}_{i}", label_visibility="collapsed")
...
def adicionar_produto(nome, preco, desc_curta, desc_longa, link_imagem, disponivel, cashback_percent_prod=0.0):
    ...
    novo_id = df['ID'].max() + 1 if not df.empty and not pd.isna(df['ID'].max()) else 1
    ...
    return write_csv_to_github(df, SHEET_NAME_CATALOGO, f"Adicionar produto: {nome} (ID: {novo_id})")
...
def atualizar_produto(id_produto, nome, preco, desc_curta, desc_longa, link_imagem, disponivel, cashback_percent_prod=0.0):
    ...
    if not index_to_update.empty:
        idx = index_to_update[0]
        df.loc[idx, ['NOME', 'PRECO', 'DESCRICAOCURTA', 'DESCRICAOLONGA', 'LINKIMAGEM', 'DISPONIVEL', 'CASHBACKPERCENT']] = [nome, str(preco).replace('.', ','), desc_curta, desc_longa, link_imagem, disponivel, str(cashback_percent_prod).replace('.', ',')]
...
def criar_promocao(id_produto, nome_produto, preco_original, preco_promocional, data_inicio, data_fim):
    ...
    nova_linha = {'ID_PROMOCAO': int(time.time()), 'ID_PRODUTO': str(id_produto), 'NOME_PRODUTO': nome_produto, 'PRECO_ORIGINAL': str(preco_original), 'PRECO_PROMOCIONAL': str(preco_promocional).replace('.', ','), 'STATUS': "Ativa", 'DATA_INICIO': data_inicio, 'DATA_FIM': data_fim}
    df = pd.concat([df, pd.DataFrame([nova_linha])], ignore_index=True)
...
def atualizar_promocao(id_promocao, preco_promocional, data_inicio, data_fim, status):
    ...
    if not index_to_update.empty:
        idx = index_to_update[0]
        df.loc[idx, ['PRECO_PROMOCIONAL', 'DATA_INICIO', 'DATA_FIM', 'STATUS']] = [str(preco_promocional).replace('.', ','), data_inicio, data_fim, status]
...
def extract_customer_cashback(itens_json_string):
    ...
    s = s.replace('""', '"').replace('\\"', '"') 
    try: return json.loads(s).get("cliente_saldo_cashback", 0.0)
    except Exception:
        try: return ast.literal_eval(itens_json_string).get("cliente_saldo_cashback", 0.0)
        except Exception: return 0.0
...
with tab_pedidos:
    ...
    if df_pedidos_raw.empty:
        ...
        if texto_filtro.strip():
            ...
            df_filtrado = df_filtrado[df_filtrado.apply(lambda row: texto_filtro in str(row['NOME_CLIENTE']).lower() or texto_filtro in str(row['ITENS_JSON']).lower(), axis=1)]
        ...
        if pedidos_pendentes.empty: st.info("Nenhum pedido pendente encontrado.")
            ...
            for _, pedido in pedidos_pendentes.sort_values(by="DATA_HORA", ascending=False).iterrows():
                with st.expander(f"Pedido de **{pedido['NOME_CLIENTE']}** - {pd.to_datetime(pedido['DATA_HORA']).strftime('%d/%m/%Y %H:%M')} - Total: R$ {pedido['VALOR_TOTAL']}"):
                    ...
                    st.markdown("---")
                    cashback_a_creditar = calcular_cashback_a_creditar(pedido.get('ITENS_JSON'), df_catalogo_pedidos)
                    ...
                    progresso_separacao = exibir_itens_pedido(pedido['ID_PEDIDO'], pedido.get('ITENS_JSON'), df_catalogo_pedidos)
                    st.progress(progresso_separacao / 100, text=f"**Progresso de Separação:** {progresso_separacao}%")
                    if st.button("✅ Finalizar Pedido", key=f"finalizar_{pedido['ID_PEDIDO']}", disabled=progresso_separacao != 100):
        ...
        if pedidos_finalizados.empty: st.info("Nenhum pedido finalizado encontrado.")
            ...
            for _, pedido in pedidos_finalizados.sort_values(by="DATA_HORA", ascending=False).iterrows():
                with st.expander(f"Pedido de **{pedido['NOME_CLIENTE']}** - {pd.to_datetime(pedido['DATA_HORA']).strftime('%d/%m/%Y %H:%M')} - Total: R$ {pedido['VALOR_TOTAL']}"):
                    ...
                    if col_reverter.button("↩️ Reverter para Pendente", key=f"reverter_{pedido['ID_PEDIDO']}", use_container_width=True):
                        if atualizar_status_pedido(pedido['ID_PEDIDO'], "PENDENTE", pd.DataFrame()): st.success(f"Pedido {pedido['ID_PEDIDO']} revertido."); st.session_state['data_version'] += 1; st.rerun()  
                        else: st.error("Falha ao reverter status.")
                    if col_excluir.button("🗑️ Excluir Pedido", type="primary", key=f"excluir_{pedido['ID_PEDIDO']}", use_container_width=True):
                        if excluir_pedido(pedido['ID_PEDIDO']): st.success(f"Pedido {pedido['ID_PEDIDO']} excluído!"); st.session_state['data_version'] += 1; st.rerun()  
                        else: st.error("Falha ao excluir o pedido.")
                    st.markdown("---")
                    exibir_itens_pedido(pedido['ID_PEDIDO'], pedido.get('ITENS_JSON'), df_catalogo_pedidos)
...
with tab_promocoes:
    ...
    with st.expander("➕ Criar Nova Promoção"):
        with st.form("form_nova_promocao", clear_on_submit=True):
            if df_produtos_catalogo.empty: st.warning("Nenhum produto cadastrado.")
            else:
                opcoes_produtos_promo = df_produtos_catalogo.apply(lambda r: f"{r['ID']} - {r['NOME']}", axis=1).tolist()
                ...
                if produto_selecionado_promo_str:
                    ...
                    produto_promo = df_produtos_catalogo[df_produtos_catalogo['ID'] == id_produto_promo].iloc[0]
                    try: preco_original = float(str(produto_promo.get('PRECO', '0.01')).replace(',', '.'))
                    except: preco_original = 0.01
                    st.caption(f"Preço Original: R$ {preco_original:.2f}")
                    preco_promocional = st.number_input("Preço Promocional (R$)", 0.01, preco_original, format="%.2f")
                    data_inicio, data_fim = st.date_input("Data de Início", date.today()), st.date_input("Data de Fim", date.today())
                    if st.form_submit_button("Criar Promoção"):
                        if preco_promocional > 0 and data_fim >= data_inicio:
                            if criar_promocao(id_produto_promo, produto_promo.get('NOME'), preco_original, preco_promocional, data_inicio, data_fim): st.success(f"Promoção para '{produto_promo.get('NOME')}' criada!"); st.session_state['data_version'] += 1; st.rerun()
                            else: st.error("Falha ao criar promoção.")
                        else: st.warning("Preço ou datas inválidas.")
    st.markdown("---")
    st.subheader("📝 Gerenciar Promoções Ativas")
    if df_promocoes.empty: st.info("Nenhuma promoção cadastrada.")
    else:
        opcoes_promocoes = df_promocoes.apply(lambda r: f"{r['ID_PROMOCAO']} - {r['NOME_PRODUTO']} ({r['STATUS']})", axis=1).tolist()
        promocao_sel_str = st.selectbox("Selecione a Promoção para Editar", opcoes_promocoes)
        if promocao_sel_str:
            id_promo_sel = int(promocao_sel_str.split(' - ')[0])
            promo_atual = df_promocoes[df_promocoes['ID_PROMOCAO'] == id_promo_sel].iloc[0]
            with st.form("form_editar_promocao"):
                st.info(f"Editando promoção ID: {id_promo_sel} para '{promo_atual['NOME_PRODUTO']}'")
                try: preco_promo_float = float(str(promo_atual['PRECO_PROMOCIONAL']).replace(',', '.'))
                except: preco_promo_float = 0.0
                id_prod_rel = pd.to_numeric(promo_atual['ID_PRODUTO'], errors='coerce')
                preco_max_promo = preco_promo_float
                if not df_produtos_catalogo.empty and not pd.isna(id_prod_rel) and not df_produtos_catalogo[df_produtos_catalogo['ID'] == int(id_prod_rel)].empty:
                    try: preco_max_promo = float(str(df_produtos_catalogo[df_produtos_catalogo['ID'] == int(id_prod_rel)].iloc[0]['PRECO']).replace(',', '.'))
                    except: pass
                try: data_inicio_default = datetime.strptime(str(promo_atual['DATA_INICIO']), '%Y-%m-%d').date()
                except: data_inicio_default = date.today()
                try: data_fim_default = datetime.strptime(str(promo_atual['DATA_FIM']), '%Y-%m-%d').date()
                except: data_fim_default = date.today()

                edit_preco_promocional = st.number_input("Novo Preço Promocional (R$)", 0.01, preco_max_promo, format="%.2f", value=preco_promo_float)
                edit_data_inicio, edit_data_fim = st.date_input("Data de Início", value=data_inicio_default), st.date_input("Data de Fim", value=data_fim_default)
                edit_status = st.selectbox("Status", ['Ativa', 'Inativa', 'Expirada'], index=['Ativa', 'Inativa', 'Expirada'].index(promo_atual['STATUS']))
                col_up, col_del = st.columns(2)
                if col_up.form_submit_button("💾 Salvar Alterações", type="primary"):
                    if edit_preco_promocional > 0 and edit_data_fim >= edit_data_inicio:
                        if atualizar_promocao(id_promo_sel, edit_preco_promocional, edit_data_inicio, edit_data_fim, edit_status): st.success(f"Promoção ID {id_promo_sel} atualizada!"); st.session_state['data_version'] += 1; st.rerun()
                        else: st.error("Falha ao atualizar promoção.")
                    else: st.warning("Verifique preço ou datas.")
                if col_del.form_submit_button("🗑️ Excluir Promoção"):
                    if excluir_promocao(id_promo_sel): st.success(f"Promoção ID {id_promo_sel} excluída!"); st.session_state['data_version'] += 1; st.rerun()
                    else: st.error("Falha ao excluir promoção.")

# --- ABA DE CUPONS (COM CORREÇÃO DO BUG) ---
with tab_cupons:
    ...
    with st.expander("➕ Criar Novo Cupom", expanded=True):
        with st.form("form_novo_cupom", clear_on_submit=True):
            ...
            with col1:
                novo_codigo = st.text_input("Código do Cupom (ex: BEMVINDO10)", key="cupom_codigo").upper()
            ...
            with col3:
                sem_validade = st.checkbox("Sem data de validade", key="cupom_sem_validade")
            ...
            with col4:
                novo_valor_minimo = st.number_input("Valor mínimo da compra para uso (R$)", min_value=0.0, format="%.2f", key="cupom_valor_minimo")

            uso_ilimitado = st.checkbox("Uso ilimitado", key="cupom_uso_ilimitado")
